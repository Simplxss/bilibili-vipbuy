...
def crop_image(img: Image.Image, x, y, width, height):
    """裁剪图片"""
    return img.crop((x, int(y), int(x + width), int(y + height)))


def paste_image(new_img: Image.Image, img: Image.Image, x, y, width, height):
    """粘贴图片到new_img中"""
    new_img.paste(img, (x, int(y), int(x + width), int(y + height)))


def restore_image(img: Image.Image):
    """还原打乱的验证码图片"""
...
def calculate_offset(img1: Image.Image, img2: Image.Image):
    """计算偏移量"""
